...
Tools generate output files that must be returned to the user. Since SkyLab can support many tools, it handles output files differently for each tool. Modules must implement the \textit{handle\_output\_files} method of the \textit{P2CToolGeneric} class which is the super class of modules. This method is typically implemented by downloading files from the cluster running the tool to SkyLab's \textit{output} directory using sftp. If a tool has multiple output files, these are zipped into one file first before downloading. The user will then be given links to download the output files (Figure ~\ref{fig:jsmol}).
