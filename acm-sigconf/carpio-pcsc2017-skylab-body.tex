...
Access to SkyLab is restricted to individuals with \textit{@up.edu.ph} email only (hosted by Google). This is implemented  using the django-allauth\footnote{http://www.intenct.nl/projects/django-allauth/} package. Tables used with \textit{socialaccount\_} prefix, as shown in Figure ~\ref{fig:dbschema}, are used for authentication. If a user is logged in to this mail, SkyLab can already be accessed.
