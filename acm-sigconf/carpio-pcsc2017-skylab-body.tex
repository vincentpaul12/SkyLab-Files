...
Access to SkyLab is restricted to individuals with \textit{@up.edu.ph} email (hosted by Google) only. This is implemented  using the django-allauth\footnote{http://www.intenct.nl/projects/django-allauth/} package. Tables used with \textit{socialaccount\_} prefix, as shown in Figure ~\ref{fig:dbschema}, are used for authentication. If a user is logged in to this email, SkyLab can directly be accessed.
...
\begin{figure}			
	...
	\captionof{figure}{MPI cluster creation. A cluster named \textit{cmsc165\_image\_processing} is being created with the \textit{Impi} tool checked to be activated.}			
\end{figure}	
...
\begin{figure}			
	...
	\captionof{figure}{MPI cluster details view. The detailed view of the \textit{cmsc165\_image\_processing} MPI cluster with a list of toolsets that can be activated.}			
\end{figure}	
...
\begin{figure}			
	...
	\captionof{figure}{Task detail view. The user can view results of tasks. JSmol \cite{IJCH:IJCH201300024} renders the compatible output files from a GAMESS task.}			
\end{figure}	
