...
Figure ~\ref{fig:sysarch} shows the layers on which SkyLab is built on. At the bottom layer is \textit{P2C} which provides the cloud infrastructure. \textit{vCluster} is for on-demand provisioning and termination of MPI clusters. \textit{p2c-tools} is the command-line tool for activating the required HPC tool.
	...
	\begin{figure*}[ht]
      ...
      \caption{\label{fig:dbschema}MariaDB database schema generated from Skylab's data models (see modes.py).}
    \end{figure*}
...
All tool classes must inherit from the \textit{P2CToolGeneric} class (see basetool.py) and override the following methods to handle tool specific operations. Listing 1 shows an example implementation for \textit{run\_tool}.
...
\begin{itemize}
	...
	\item \textit{run\_tool} - Method that gets executed in a task. Must be overridden.
...
Traditional usage of HPC applications requires users to learn job submission systems like Torque\footnote{http://www.adaptivecomputing.com/products/open-source/torque/} or Slurm \footnote{https://slurm.schedmd.com/} that are used from the command line. Scientific workflow systems make it easier for users to use their tools without mastering the command line.
...
Taverna \cite{hull2006taverna} is another scientific workflow system that is gaining popularity and is currently an Apache Foundation incubating project.  
