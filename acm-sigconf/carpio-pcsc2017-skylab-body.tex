...
Tools generate output files that must be returned to the user. Since SkyLab can support many tools, handling output files depend on the tool. The \textit{handle\_output\_files} method of the \textit{P2CToolGeneric} class must be implemented on the module of the tool. SkyLab downloads the files from the cluster running the tool to its output directory using sftp. Multiple output files zipped into one file.
